...
class DataLoader(ABC):
    ...
    def __init__(self, dataset, use_batch=True, batch_size=100, n_batches=0, shuffle=False, sampling_ratio=1, mnist = False , mean = (0.5,0.5,0.5), std = (0.5,0.5,0.5)):
        """
    ...
    def load(self):
        ...
        if self.sampling_ratio >= 1:
            ...
            set_random_seed(self.cc.settings['general']['seed'] + self.cell_number+1,
    ...
    def transform(self):
        return transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=self.mean,
                                                                               std=self.std)])
