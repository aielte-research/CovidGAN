...
class FIDCalculator(ScoreCalculator):
    ...
    def __init__(self, imgs_original, batch_size=64, dims=2048, n_samples=10000, cuda=True, verbose=True):
        """
        ...
        if self.cc.settings['dataloader']['dataset_name'] == 'mnist':
            ...
            self.dims = 3  # For covid the dimension of feature map is 3
        else:
            self.dims = dims
    ...
    def calculate(self, imgs, exact=True):
        ...
        if self.cc.settings['dataloader']['dataset_name'] == 'mnist':    # Gray dataset
            ...
            file_path = str(pathlib.Path(__file__).parent.absolute())
            model.load_state_dict(torch.load(file_path + '/../../ensemble_optimization/output/networks/covid_classifier.pt'))
        ...
        tvd = 0.5 * sum(abs(f1 - f2) for f1, f2 in zip(freq1, freq2)) #1/2 * sum for each label |freq1[label] - freq2[label]|
    ...
    def get_activations(self, images, model):
        ...
        for i in range(n_batches):
            ...
            with torch.no_grad():
                ...
                if self.cc.settings['dataloader']['dataset_name'] == 'covidqu':
                  #TODO IF batch.Size()!= original image tensor size, then reshape  
                  batch = torch.reshape(batch, (len(batch), 1, 128, 128)) 
                  pred = model(batch)
                else:
                  pred = model(batch)[0]
                
                if self.cc.settings['dataloader']['dataset_name'] not in ('mnist', 'covidqu') :
        ...
        #pred_arr is model.output_size*n_used_imgs size
        return pred_arr
    ...
    def get_frequencies_of_activations(self, activations):
        #Calculates image activation frequencies
        frequencies = 10 * [0] #shouldn't this have model.output_size dimensions?
