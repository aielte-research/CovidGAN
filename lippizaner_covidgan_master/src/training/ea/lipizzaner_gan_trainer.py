...
class LipizzanerGANTrainer(EvolutionaryAlgorithmTrainer):
    ...
    def __init__(self, dataloader, network_factory, population_size=10, tournament_size=2, mutation_probability=0.9,
        ...
        if 'score' in self.settings and self.settings['score'].get('enabled', calc_mixture):
            if self.cc.settings['dataloader']['dataset_name'] == 'mnist':
                self.score_calc = ScoreCalculatorFactory.create(mean = 0.5, std = 0.5)
            else:
        ...
        self.log_history = self.cc.settings['network'].get('history', False)
        
        #Potentially we could also log the FID score
    ...
    def train(self, n_iterations, stop_event=None):
        ...
        for iteration in range(n_iterations):
            ...
            if self.evaluate_subpopulations_every == 1 or (iteration % self.evaluate_subpopulations_every == 0):
                all_generators = self.neighbourhood.all_generators(self.individuals_sampling_size) #Includes local generators too 
                all_discriminators = self.neighbourhood.all_discriminators(self.individuals_sampling_size) #This is a Population
                ...
                new_populations = {} #This is a dictionary

                # Create random dataset to evaluate fitness in each iterations
                
                fitness_samples = self.generate_random_fitness_samples(self.fitness_sample_size) #on cuda
            ...
            #    print("Individual updates recieved before: ", individual.updates_recieved)
            #    print("losses: ", individual.history)
            #for individual in new_populations[TYPE_DISCRIMINATOR].individuals:
            #    print("Individual name: ", individual.name)
            #    print("Individual updates recieved before: ", individual.updates_recieved)
            #    print("losses: ", individual.history)
            ...
            while self.batch_number < len(loaded):
                ...
                #Complete an optimizing step with the generators
                ...
                if self._discriminator_skip_each_nth_step == 0 or self.batch_number % (
                    ...
                    #Wouldn't I need an else here? Currently I think it completes a step in every n_th iteration
            ...
            print("\nLogging history of new populations")
            if self.log_history:
                genloss = []
                discloss = []
                for individual, losses in self.batch_histories.items():
                  individual.add_loss(losses)
                  if individual.genome.name in ('Generator', 'GeneratorSequential'):
           ...
           #       print("="*20)
            ...
            if self._enable_selection:
                ...
                #It seems to me that this step is not necessary, because earlier we compute local_generator fitness
                #with all_generator fitness
    ...
    def update_genomes(self, population_attacker, population_defender, input_var, loaded, data_iterator, training_epoch):
        ...
        individual_defender =  random.choice(population_defender.individuals)
        defender = individual_defender.genome
    ...
    def evaluate_fitness(population_attacker, population_defender, input_var, fitness_mode, split=False):
        ...
        for individual_attacker in population_attacker.individuals:
            _logger.debug(' Attacker {}'.format(individual_attacker.id))
            ...
            for individual_defender in population_defender.individuals:
                ...
                if split:
                    ...
                    #TODO Itt még nem találom a compute loss against function, hogy megnézzem hogyan számol 
                    #Input_var az eredeti képekből, egy fitness_sample_size-nyi batch
        ...
        if self.score_calc is not None:
            ...
            if self.neighbourhood.grid_size == 1:
                ...
                dataset = MixedGeneratorDataset(best_generators,
                                                ...
                                                self.score_sample_size,
                                                self.cc.settings['trainer']['mixture_generator_samples_mode'])  #This is put on cuda device if master: cuda: True
                #score is the fid measurement of the orig datasets img variety and generated dataset's variety
                self.score = self.score_calc.calculate(dataset)[0]
                self._logger.info(
                    'Mixture weight mutation - Generation: {} \tScore of new weights: {}\tNew weights: {}.'.format(best_generators.individuals[0].iteration,
                                                                                               self.score,
                                                                                               self.neighbourhood.mixture_weights_generators))
            else:
                # Mutate mixture weights
    ...
    def generate_random_fitness_samples(self, fitness_sample_size):
        ...
        if fitness_sample_size < batch_size:
            ...
            while fitness_sample_size >= batch_size:
                ...
                fitness_sample_size -= curr_data.size(0)#batch_size
        ...
        sampled_data = sampled_data[:orig_size] #I know it"s a little bit kókánymunka but well
