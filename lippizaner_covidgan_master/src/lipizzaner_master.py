...
class LipizzanerMaster:
    ...
    def _gather_results(self):
        ...
        log_history =  self.cc.settings['network'].get('history', False)
        
        scores = []
        for (node, generator_pop, discriminator_pop, weights_generator, weights_discriminator, gen_history, disc_history, fid_history ) in results:
            ...
            try:
                ...
                for generator in generator_pop.individuals:
                    ...
                    if log_history:
                        with open(os.path.join(output_dir, 'generator-{}-individual-history.pkl'.format(source)), 'wb') as file:
                            pickle.dump(generator.history, file)
                ...
                for discriminator in discriminator_pop.individuals:
                    ...
                    if log_history:
                        with open(os.path.join(output_dir, 'discriminator-{}-individual-history.pkl'.format(source)), 'wb') as file:
                            pickle.dump(discriminator.history, file)
                            
                # Save images
                if log_history:
                    with open(os.path.join(output_dir, 'All_generator_history-{}.pkl'.format(node_name)), 'wb') as file:
                        pickle.dump(gen_history, file)
                    with open(os.path.join(output_dir, 'All_discriminator_history-{}.pkl'.format(node_name)), 'wb') as file:
                        pickle.dump(disc_history, file)
                    with open(os.path.join(output_dir, 'FID-history-{}.pkl'.format(node_name)), 'wb') as file:
                        pickle.dump(fid_history, file)
                ...
                if self.cc.settings['master']['calculate_score']:
                    if self.cc.settings['dataloader']['dataset_name'] == 'mnist':
                        calc = ScoreCalculatorFactory.create(mean = 0.5, std = 0.5)
                    else:
